...
3. **Tmp paths must be portable.** Use `.seo-tmp/` inside the working dir, never `/tmp/`, which does not exist on Windows.
...
5. **The Write tool syntax-gates `.py` files via ruff.** Valid Python passes; a parse error blocks with a located message. (The old blocked-when-ruff-missing failure was fixed 2026-07-23: the hook now fails open when the engine is absent, and the apply script bootstraps ruff.) Node generator scripts are not gated.
