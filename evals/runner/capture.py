...
def _home_relative_repo(repo):
    # task.yml is tracked in a public repo, so the machine's home prefix stays out of it.
    if not repo:
        return repo
    home = os.path.normpath(os.path.expanduser("~"))
    normalized = os.path.normpath(repo)
    if not normalized.lower().startswith(home.lower() + os.sep):
        return repo
    return "~/" + normalized[len(home) + 1:].replace(os.sep, "/")
...
def capture_task(tasks_dir, task_id, task_class, mode, prompt_file, repo=None):
    ...
    with open(os.path.join(task_dir, "task.yml"), "w", encoding="utf-8") as handle:
        handle.write(_task_yml_text(task_id, task_class, mode, _home_relative_repo(repo), pinned_commit))
