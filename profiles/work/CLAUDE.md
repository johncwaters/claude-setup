...
4. Agent type: read-only searches and codebase sweeps go to `Explore`, not `general-purpose`. Every spawn, including `general-purpose`, pins `model` explicitly; `general-purpose` gets `sonnet` unless the task hits the opus reserved list.
