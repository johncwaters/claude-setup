...
Codex dispatch mechanics: write the full prompt to a temp file and run `codex exec --dangerously-bypass-approvals-and-sandbox -m gpt-5.6 < <file>`; if GPT-5.6 is unavailable or errors, retry once with `-m gpt-5.5` and say which model handled it. Codex runs standalone with its own CLI tools and no shared context or memory; read the final assistant message from stdout and judge it yourself before treating it as a completion claim.
