...
| Anything not listed above (`haiku`, `gpt-5.5`, `gpt-5.4`, `-mini`, `-codex`, older grok) | - | - | - | Banned | This table is an allow-list: a model absent from it is never used, no exceptions, overrides every other rule in this file |
...
2. Work kept off Codex by rule 1, or after a 1a fallback: `opus` is the default Claude coding tier, including implementation, multi-file changes, debugging, review, and any task of uncertain complexity. Drop to `sonnet` only for one-shot slices with simple, checkable output (formulaic single-file edits, simple lookups, routine read-only sweeps) where opus changes nothing; the sticker gap is mostly an illusion on anything iterative, since the cheaper tier burns the difference in extra turns and re-reads, so anything open-ended, multi-step, or uncertain goes `opus`. Never `haiku`.
